...
def semantic_search(
    ...
    filtered_results = [
        ...
        if r["document_id"] in user_document_ids and r["distance"] <= MAX_L2_DISTANCE_THRESHOLD
    ]
    ...
    document_filenames = {
        ...
        for doc in db.query(Document).filter(Document.id.in_(user_document_ids)).all()
    }

    final_results = []
    for r in top_results:
        relevance_score = max(0.0, 1.0 - (r["distance"] / (MAX_L2_DISTANCE_THRESHOLD * 2)))
        final_results.append(SearchResult(
            document_id=r["document_id"],
            document_filename=document_filenames.get(r["document_id"], "Unknown"),
            chunk_text=r["chunk_text"],
            relevance_score=round(relevance_score, 3)
        ))

    return SearchResponse(query=search_query.query, results=final_results)